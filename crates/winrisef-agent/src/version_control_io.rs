...
pub(super) async fn write_preview_stream(session: &Session, request_id: u32, content: PreviewContent) -> anyhow::Result<()> {
    ...
    let mut send = session.open_uni().await.context("failed to open preview stream")?;
    send.write_all(&(metadata.len() as u32).to_be_bytes()).await?;
...
pub(super) async fn read_frame(recv: &mut RecvStream) -> anyhow::Result<Option<BridgeInput>> {
    ...
    let Some(first) = recv.read(&mut prefix[..1]).await? else { return Ok(None); };
    anyhow::ensure!(first == 1, "invalid version-control frame prefix");
    recv.read_exact(&mut prefix[1..]).await?;
    let len = u32::from_be_bytes(prefix) as usize;
    anyhow::ensure!(len > 0 && len <= MAX_FRAME_BYTES, "invalid version-control frame length");
    let mut bytes = vec![0_u8; len];
    recv.read_exact(&mut bytes).await?;
    Ok(Some(serde_json::from_slice(&bytes)?))
}

pub(super) async fn write_frame<T: Serialize>(send: &mut SendStream, value: &T) -> anyhow::Result<()> {
    let bytes = serde_json::to_vec(value)?;
    anyhow::ensure!(!bytes.is_empty() && bytes.len() <= MAX_FRAME_BYTES, "version-control response is too large");
