...
Every request after hello has a numeric `requestId`. Repository, diff, file, revision, and export references are created or validated by the Agent. The browser cannot submit a filesystem path. SVN accepts `empty` as revision 0, decimal `r<N>` commit IDs, and the working tree only as the right-hand revision; stash, index, malformed IDs, and a left-hand working tree are rejected instead of being reinterpreted. History and file pages are reduced until their serialized response fits the control-frame budget.
...
Diff sessions keep one bounded metadata record per changed path and the Agent retains at most three sessions. The Agent locates `svn.exe` once per process; `svn info --xml` validates that executable and identifies the selected working copy. A working-copy Diff combines `svn status --xml --verbose` for worktree-only/conflict metadata with `svn diff --summarize` for authoritative A/M/D/property metadata and node kinds, including deleted directories. Historical comparisons use only the summary. Each SVN Diff runs one complete `svn diff --git` and incrementally parses its stdout through a fixed 64KiB read buffer; the Agent never reconstructs the complete command output. Text-hunk counters and binary metadata remain complete, while an individual Patch body retains at most 2MiB and the three-range Patch cache retains at most 32MiB. Malformed byte sequences are replaced only in retained review text, so one non-UTF-8 file cannot fail the whole workspace. Unversioned files receive only an 8KiB NUL-byte probe while Diff metadata is built; their complete body is read only when that file is previewed. `refresh-repository` invalidates worktree-sensitive status, summary, Patch, and source caches. Commit-to-commit comparisons do not read or attach current worktree status. Closing the control session invalidates repository state and cancels any active export.
...
Patch mode is the default SVN review path. It streams the retained per-file Patch in `original` and leaves `modified` empty. A Patch body not resident in the global cache starts one path-scoped `svn diff` only when that file is opened; the result then enters the Diff Session preview cache. Patch mode never starts `svn cat`. The browser renders only hunk context and changed lines with real old/new line numbers; omitted spans are placeholders. Patch decoding is lossy by design so malformed bytes remain inspectable without aborting unrelated files.

Full mode is limited to 2MiB per side. NUL/binary and oversized files do not produce full source bodies; malformed UTF-8 is decoded with replacement characters. Symlinks expose the link value and are not followed. LFS pointer files remain pointers.
...
- Ordinary SVN stdout and stderr are drained concurrently under a 45-second command deadline and 16MiB stdout ceiling. Complete `svn diff --git` uses a separate 120-second deadline and 512MiB streamed-processing ceiling; that ceiling is counted, not allocated. A timeout, read failure, or processing overflow terminates the child immediately. Retained Patch memory remains bounded independently of the processing ceiling.
